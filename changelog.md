### Added
An interactive map on openstreetmap
clickable map markers
light box gallary
inserted another interactive map with pin-point location
Errors on the contact form
inserted css to make products more formal looking 
interactive images one the products page 
backlinks


### Changed
Navigation options
the second picture in the about us webpage
navigation adjustments according to the screensize of a mobile,tablet and desktop



### Fixed
Navigation now moves vertically according to the mobile screensize
The image names
Product functionallity 
conntact form from multiple forms into one single form structure

## Removed
